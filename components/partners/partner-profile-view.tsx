...
import { PartnerUpcomingSessions } from "./partner-upcoming-sessions"
...
function backHrefForKind(kind: PartnerWithPrograms["kind"]): string {
  if (kind === "brand") return "/journal"
  if (kind === "artist") return "/#arts"
  return "/#guides"
}

function backLabelForKind(kind: PartnerWithPrograms["kind"]): string {
  if (kind === "brand") return "Journal"
...
export function PartnerProfileView({ person, sessions, sido }: PartnerProfileViewProps) {
    ...
    <div className="mx-auto max-w-4xl px-6 py-12 lg:px-10 lg:py-16">
      ...
      {person.kind !== "brand" && (
        <div className="mt-16 border-t border-border pt-16">
          <PartnerUpcomingSessions sessions={sessions} />
        </div>
      )}
    </div>
  )
