...
import { PartnerSessionListItem } from "./partner-session-list-item"

type PartnerUpcomingSessionsProps = {
  sessions: SessionWithRelations[]
}

export function PartnerUpcomingSessions({ sessions }: PartnerUpcomingSessionsProps) {
  return (
      ...
      {sessions.length === 0 ? (
        ...
        <ul className="space-y-3">
          {sessions.map((session) => (
            <PartnerSessionListItem key={session.id} session={session} />
          ))}
        </ul>
      )}
    </section>
