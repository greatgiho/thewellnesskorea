...
import type { PartnerRegistrationStatus, PartnerWithPrograms } from "@/lib/partners/types"
import {
...
function personMatchesPhilosophy(person: PartnerWithPrograms, paths: PathKey[]) {
...
function personMatchesSearch(person: PartnerWithPrograms, query: string) {
  ...
  const filtered = useMemo(() => {
    const list = partners.filter(
      (person) =>
        ...
        partnerMatchesKind(person, kindFilter) &&
        personMatchesPhilosophy(person, philosophyFilters) &&
        personMatchesStatus(person, statusFilter),
    )
    return sortPartnersByName(list)
  }, [partners, search, kindFilter, philosophyFilters, statusFilter])
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-muted-foreground">
        {filtered.length} of {partners.length} shown · sorted by name (EN), then registration
        date
      </p>
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="relative size-10 overflow-hidden rounded-lg bg-secondary">
                        <Image
                          src={getPartnerPhotoUrl(p.photo_path)}
                          alt=""
                          fill
