...
- **Partner (강사/파트너)** — 셀프 등록 + 포털
- **Admin (운영자)** — 콘텐츠·스케줄·예약 운영

## 2. 공개 사이트 IA (Visitor 멘탈 모델)

- **글로벌 내비게이션** (navbar)
  - Philosophy → 철학 섹션
  - Wellness Guides → 가이드 섹션
  - Artists → 아티스트 섹션
  - Schedule → 스케줄 섹션
  - Journal → 저널 (별도 페이지)
- **홈 내러티브** (스크롤 순서)
...
- **상태 enum** — Session(processing/confirmed/cancelled) · Partner 등록(admin/draft/submitted/approved/rejected)
...
- **운영 내비게이션**
  - Partners — 목록 · 등록 · 상세/검토 · 편집(+계정)
  - Schedule — 주/일/월 그리드, 세션 생성·확정·발행
  - Bookings — 세션별 예약 · 대기자
  - Waitlist — 대기자 개요
  - Journal — 글 목록 · 작성 · 편집
- **운영 대상 = §3 콘텐츠 타입의 CRUD/상태 전이**

## 8. IA ↔ Sitemap 차이 (요약)

- **Sitemap** = 존재하는 URL의 트리 (site-map-and-flows.md)
- **IA** = 위 콘텐츠를 *어떻게 묶고 분류하고 탐색하는가* — 글로벌 내비/푸터 그룹, 콘텐츠 타입 관계, 교차 분류(Five Paths 등)
